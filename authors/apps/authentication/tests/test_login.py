...
class TestLogin(APITestCase):
    ...
    def setUp(self):
        ...
        response = self.client.post(
            self.signup_url, data=user_data, format="json"
        )
    ...
    def test_wrong_email(self):
        response = self.client.post(
            self.login_url,
            data={
                "user": {"email": "jane@bg.com", "password": "simplepassword"}
            },
            format="json",
        )
    ...
    def test_wrong_password(self):
        response = self.client.post(
            self.login_url,
            data={
                "user": {"email": "janee@bg.com", "password": "wrongpassword"}
            },
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_empty_email(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            response.data.get("errors").get("email")[0],
